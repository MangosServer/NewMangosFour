...
bool PlayerCondition::Meets(Player const* player, Map const* map, WorldObject const* source, ConditionSource conditionSourceType) const
{
    ...
    switch (m_condition)
    {
        ...
        case CONDITION_AD_COMMISSION_AURA:
        {
            ...
            {
                if ((itr->second->GetSpellProto()->HasAttribute(SPELL_ATTR_CASTABLE_WHILE_MOUNTED) || itr->second->GetSpellProto()->HasAttribute(SPELL_ATTR_ABILITY)) && itr->second->GetSpellProto()->GetSpellVisual(0) == 3580)
                {
                    return true;
                }
            }
