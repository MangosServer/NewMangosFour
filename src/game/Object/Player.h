...
// The MoP skill block is 7 parallel arrays
// (LINEID/STEP/RANK/STARTING_RANK/MAX_RANK/MODIFIER/TALENT),
...
enum TransferAbortReason
{
    ...
    TRANSFER_ABORT_NOT_FOUND                    = 0x00,     // Transfer Aborted: instance not found
    ...
    TRANSFER_ABORT_UNIQUE_MESSAGE               = 0x0E,     // Until you've escaped TLK's grasp, you cannot leave this place!
    TRANSFER_ABORT_TOO_MANY_REALM_INSTANCES     = 0x18,     // Additional instances cannot be launched, please try again later.
    TRANSFER_ABORT_NEED_GROUP                   = 0x0F,     // 3.1
    TRANSFER_ABORT_NOT_FOUND2                   = 0x0A,     // 3.1
    TRANSFER_ABORT_NOT_FOUND3                   = 0x0C,     // 3.1
    TRANSFER_ABORT_NOT_FOUND4                   = 0x12,     // 3.2
    TRANSFER_ABORT_REALM_ONLY                   = 0x1F,     // All players on party must be from the same realm.
    TRANSFER_ABORT_MAP_NOT_ALLOWED              = 0x0D,     // Map can't be entered at this time.
    TRANSFER_ABORT_LOCKED_TO_DIFFERENT_INSTANCE = 0x16,     // 4.0.1
    TRANSFER_ABORT_ALREADY_COMPLETED_ENCOUNTER  = 0x06,     // 4.0.1
};

namespace MopTransferPackets
{
    inline void BuildTransferAborted(WorldPacket& out, uint32 mapId,
        uint8 transferReason, uint8 argument)
    {
        bool const hasTransferArgument =
            transferReason == TRANSFER_ABORT_INSUF_EXPAN_LVL ||
            transferReason == TRANSFER_ABORT_DIFFICULTY ||
            transferReason == TRANSFER_ABORT_UNIQUE_MESSAGE;

        out.WriteBit(!hasTransferArgument);
        out.WriteBits(uint32(transferReason), 5);
        out.FlushBits();
        if (hasTransferArgument)
            out << uint8(argument);
        out << uint32(mapId);
    }
}
