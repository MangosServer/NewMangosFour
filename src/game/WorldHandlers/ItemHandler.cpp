...
void WorldSession::HandleSplitItemOpcode(WorldPacket& recv_data)
{
    ...
    // Parse the 18414 body before applying the established authoritative
    // inventory validation and split operation below.
    MopItemPackets::SplitItemRequest request;
    if (!MopItemPackets::ParseSplitItem(recv_data, request))
        return;

    uint8 const srcbag = request.sourceBag;
    uint8 const srcslot = request.sourceSlot;
    uint8 const dstbag = request.destinationBag;
    uint8 const dstslot = request.destinationSlot;
...
void WorldSession::HandleSwapInvItemOpcode(WorldPacket& recv_data)
{
    ...
    // The client appends two compressed update records after these slots.
    MopItemPackets::SwapInvItemRequest request;
    if (!MopItemPackets::ParseSwapInvItem(recv_data, request))
        return;
...
void WorldSession::HandleSwapItem(WorldPacket& recv_data)
{
    ...
    // Cross-container swaps use a different leading-field and update-byte
    // order from base-inventory swaps.
    MopItemPackets::SwapItemRequest request;
    if (!MopItemPackets::ParseSwapItem(recv_data, request))
        return;
...
void WorldSession::HandleAutoEquipItemOpcode(WorldPacket& recv_data)
{
    ...
    // Auto-equip sends cursor slot before cursor bag and includes one
    // compressed affected-position record.
    MopItemPackets::AutoEquipItemRequest request;
    if (!MopItemPackets::ParseAutoEquipItem(recv_data, request))
        return;

    uint8 const srcbag = request.cursorBag;
    uint8 const srcslot = request.cursorSlot;
