...
namespace MopCombatLogPackets
{
    ...
    template <size_t N>
    inline void WriteGuidMask(WorldPacket& out, uint64 guid, uint8 const (&order)[N])
    {
    ...
    template <size_t N>
    inline void WriteGuidBytes(WorldPacket& out, uint64 guid, uint8 const (&order)[N])
    {
        for (uint8 index : order)
    ...
    inline void BuildPeriodicAuraLog(WorldPacket& out, PeriodicAuraLog const& log)
    {
