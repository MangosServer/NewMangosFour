 ...
 * to it. Nothing else about networking lives at this level -- accepting,
 * worker threads, buffering, backpressure and teardown all belong to the
 * shared engine underneath (src/shared/net/).
 *
 * A separate object, rather than fields on WorldThread directly, so the
 * ownership and shutdown order stay explicit.
 */
